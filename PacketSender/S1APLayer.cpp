...
u_char CS1APLayer::SearchItems(u_char proc, u_char* ppayload)
{
	...
	while(1)
	{
		...
		else
		{
			// begin: 알맞은 값을 채우시오
			/******************************/
			i += header.length; // 패킷 탐색을 위한 적절한 길이를 작성하시오. (hint: S1AP의 ITEM 포맷을 참고하시오.)
			// end
		}
...
{
	// begin: 알맞은 값을 채우시오
	m_sHeader.s1ap_pdu = S1AP_MSG_TYPE_ATTACH_REQUEST; // a type of PDU
...
{
	// begin: 알맞은 값을 채우시오
	m_sHeader.s1ap_pdu = S1AP_MSG_TYPE_AUTHEN_RESPONSE; // a type of PDU
	m_sHeader.s1ap_proc_code = S1AP_PROC_CODE_DOWNLINK_NAS_TRANS; // procedure code
	// end
...
{
	// begin: 알맞은 값을 채우시오
	m_sHeader.s1ap_pdu = S1AP_MSG_TYPE_ATTACH_COMPLETE1; // a type of PDU
...
{
	// begin: 알맞은 값을 채우시오
	m_sHeader.s1ap_pdu = S1AP_MSG_TYPE_ATTACH_COMPLETE2; // a type of PDU
