...
{
	// begin: 알맞은 값을 채우시오
	m_sHeader.sctp_srcport = htons(7977);
...
void CSCTPLayer::SetChunkData(int nlength)
{
	...
	// begin: 알맞은 값을 채우시오
	/*****88***********************************************************/
	//m_sChunk.chunk_length = ntohs(CHUNK_HEADER_SIZE + nlength);
...
BOOL CSCTPLayer::Send(u_char* ppayload, int nlength)
{
	...
	//int packet_length = m_sChunk.chunk_length + cLength; /* + [SCTP PACKET TOTAL SIZE] */
	// end
